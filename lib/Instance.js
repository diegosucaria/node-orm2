...
function Instance(Model, opts) {
	...
	var removeInstance = function (cb) {
		...
		var conditions = {};
		var fieldNames = [];
		Object.keys(opts.fieldToPropertyMap).forEach(function (key){
			fieldNames[opts.fieldToPropertyMap[key].name] = opts.fieldToPropertyMap[key].mapsTo;
		});
		for (var i = 0; i < opts.keys.length; i++) {
		     	//conditions[opts.keys[i]] = opts.data[opts.keys[i]];
            		conditions[fieldNames[opts.keys[i]]] = opts.data[opts.keys[i]];
		}
